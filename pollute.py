...
def validate_perturbation(original, perturbed):
    """验证是否发生有效扰动"""
    return any(o != p for o, p in zip(original[1:], perturbed[1:]))  # 排除序号列
        ...
        for field, prob in field_order:
            ...
            try:
                if field == 'name':
                    ...
                    row[5] = ''.join(random.choices(chars, k=len(row[5])))
